...
* Form.Elements't 1.6 jQuery içinde artık desteklenen that'sn kullanmayın. Kendi buggy olarak cehennemi yine de (IE6-8: form.elements === form).
...
* Yöntemleri isteğe bağlı alan boş bırakılırsa hiçbir şey hiç vurgulayın değil, sabit, olduğunu yoksa tetikleme başarılı
...
* Ancak bunlar yine de (varsa) hata etiketleri girişleri bağlamak için kullanılan öğesi kimlikleri, bağımlılık tamamen kaldırıldı. Bir dizi {adı, ileti, kullanarak öğesi ile} iç errorList kimliği: ileti çiftleri ile bir nesne yerine elde.
