...
> Bu belgede, browsers ve Windows Mağazası (WinJS) uygulamaları gibi JavaScript istemcilerinde SignalR sürüm 1,1 için hub API 'SI kullanımı hakkında bir giriş sunulmaktadır.
> 
...
Yukarıdaki örnekte, SignalR tarafından oluşturulan proxy 'nin başvurusu, fiziksel bir dosyaya değil, dinamik olarak oluşturulan JavaScript koduna sahiptir. SignalR, anında ara sunucu için JavaScript kodunu oluşturur ve "/SignalR/Hub" URL 'sine yanıt olarak istemciye hizmet verir. `MapHubs` yönteminizin sunucusunda SignalR bağlantıları için farklı bir temel URL belirttiyseniz, dinamik olarak oluşturulan ara sunucu dosyasının URL 'SI, "/Hub" a eklenmiş olan özel URL 'niz olur.
...
ASP.NET MVC 4 Razor görünümünde, ara sunucu dosya başvurunuz içindeki uygulama köküne başvurmak için tilde kullanın:

[!code-html[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample5.html)]

MVC 4 ' te SignalR kullanma hakkında daha fazla bilgi için bkz. [SignalR ve MVC 4 Ile çalışmaya](tutorial-getting-started-with-signalr-and-mvc-4.md)başlama.
...
Visual Studio 2012 ' de bir Web projesini hata ayıklama modunda çalıştırdığınızda ve tarayıcınız olarak Internet Explorer kullanıyorsanız, aşağıdaki çizimde gösterildiği gibi, proxy dosyasını **betik belgeleri**altında **Çözüm Gezgini** görebilirsiniz.
...
Dosyanın içeriğini görmek için **hub 'lara**çift tıklayın. Visual Studio 2012 ve Internet Explorer kullanmıyorsanız veya hata ayıklama modunda değilseniz, "/SignalR/Hub" URL 'sine giderek dosyanın içeriğini de alabilirsiniz. Örneğin, siteniz `http://localhost:56699`çalışıyorsa tarayıcınızda `http://localhost:56699/SignalR/hubs` gidin.
...
Genellikle tarayıcı `http://contoso.com`bir sayfa yüklerse, SignalR bağlantısı aynı etki alanında, `http://contoso.com/signalr`. `http://contoso.com` sayfa, etki alanları arası bağlantı olan `http://fabrikam.com/signalr`bir bağlantı yapıyorsa. Güvenlik nedenleriyle, etki alanları arası bağlantılar varsayılan olarak devre dışıdır. Etki alanları arası bağlantı kurmak için sunucuda etki alanları arası bağlantıların etkinleştirildiğinden emin olun ve bağlantı nesnesini oluştururken bağlantı URL 'sini belirtin. SignalR, [JSONP](http://en.wikipedia.org/wiki/JSONP) veya [CORS](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing)gibi etki alanları arası bağlantılar için uygun teknolojiyi kullanacaktır.

Sunucusunda, `MapHubs` yöntemini çağırdığınızda bu seçeneği belirleyerek etki alanları arası bağlantıları etkinleştirin.

[!code-csharp[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample10.cs?highlight=2)]

İstemci üzerinde, bağlantı nesnesini oluştururken (oluşturulan proxy olmadan) veya Start yöntemini (oluşturulan ara sunucu ile) çağırmadan önce URL 'YI belirtin.

**Çapraz etki alanı bağlantısını belirten istemci kodu (oluşturulan ara sunucu ile)**

[!code-javascript[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample11.js?highlight=1)]

**Çapraz etki alanı bağlantısını belirten istemci kodu (oluşturulan proxy olmadan)**

[!code-javascript[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample12.js?highlight=1)]

`$.hubConnection` oluşturucusunu kullandığınızda, otomatik olarak eklendiği için, URL 'ye `signalr` eklemeniz gerekmez (`useDefaultUrl` `false`olarak belirtmediğiniz sürece).

Farklı uç noktalara birden fazla bağlantı oluşturabilirsiniz.
...
> 
>     SignalR, JSONP veya CORS kullanımını işler. `jQuery.support.cors` true olarak ayarlamak, SignalR 'nin tarayıcının CORS 'yi desteklediğini varsaymasına neden olduğundan JSONP 'yi devre dışı bırakır.
...
Günlükleri görmek için tarayıcınızın geliştirici araçlarını açın ve konsol sekmesine gidin. Bunun nasıl yapılacağını gösteren adım adım yönergeleri ve ekran görüntülerini gösteren bir öğretici için, bkz. [ASP.NET SignalR Ile sunucu yayını-günlüğü etkinleştirme](index.md).
