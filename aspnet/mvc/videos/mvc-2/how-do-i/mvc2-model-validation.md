---
uid: mvc/videos/mvc-2/how-do-i/mvc2-model-validation
title: MVC2 - Model doğrulama | Microsoft Docs
author: microsoft
description: Bu videoda, doğrulama kurallarını tanımlamak için veri ek açıklama öznitelikleri kullanma ve istemci tarafı (AJAX) doğrulama etkinleştirme hakkında bilgi edineceksiniz.
ms.author: riande
ms.date: 03/23/2010
ms.assetid: fe3676fd-ecec-4756-8c8d-269da9754c31
msc.legacyurl: /mvc/videos/mvc-2/how-do-i/mvc2-model-validation
msc.type: video
ms.openlocfilehash: ec1581f9916ab76fedc6b25ef967b060cdfb194d
ms.sourcegitcommit: 0f1119340e4464720cfd16d0ff15764746ea1fea
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/09/2019
ms.locfileid: "59382609"
---
# <a name="mvc2---model-validation"></a>MVC2 - Model Doğrulama

tarafından [Microsoft](https://github.com/microsoft)

Bu videoda, doğrulama kurallarını tanımlamak için veri ek açıklama öznitelikleri kullanma ve istemci tarafı (AJAX) doğrulama etkinleştirme hakkında bilgi edineceksiniz.

[&#9654;(4 dakika) videosunu izleyin](https://channel9.msdn.com/Blogs/ASP-NET-Site-Videos/mvc2-model-validation)

> [!div class="step-by-step"]
> [Önceki](mvc2-stronglytyped-helpers.md)
> [İleri](mvc2-template-customization.md)
