...
[Kodu indirin](https://download.microsoft.com/download/3/9/f/39f92b37-e92e-4ab3-909e-b4ef23d01aa3/ASPNET_Data_Tutorial_75_VB.zip) veya [PDF 'yi indirin](creating-stored-procedures-and-user-defined-functions-with-managed-code-vb/_static/datatutorial75vb1.pdf)
...
Yalnızca, artık durdurulmuş olan tüm ürünleri döndüren bir saklı yordam ekleyerek başlayalım. Yeni saklı yordam dosyasını `GetDiscontinuedProducts.vb`olarak adlandırın.
...
Saklı yordamın `StoredProcedures`adlı bir `Partial` sınıf dosyası içinde `Shared` yöntemi olarak uygulandığını unutmayın. Üstelik `GetDiscontinuedProducts` yöntemi, yöntemi saklı yordam olarak işaretleyen [`SqlProcedure` özniteliğiyle](https://msdn.microsoft.com/library/microsoft.sqlserver.server.sqlprocedureattribute.aspx)birlikte tasarlanmıştır.
...
`ManagedDatabaseConstructs` projesine yönetilen bir UDF eklemek için, Çözüm Gezgini içindeki proje adına sağ tıklayın ve yeni bir öğe eklemeyi seçin. Yeni öğe Ekle iletişim kutusundan Kullanıcı tanımlı şablon ' u seçin ve yeni UDF dosyasını `udf_ComputeInventoryValue_Managed.vb`adlandırın.
...
Şimdi bu sınıfı bir derlemede derlemeniz gerekir. Komut satırından, `GetProductsWithPriceGreaterThan.vb` dosyasını kaydettiğiniz dizine gidin ve sınıf dosyasını bir derlemede derlemek için C# derleyicisini (`csc.exe`) kullanın:

[!code-console[Main](creating-stored-procedures-and-user-defined-functions-with-managed-code-vb/samples/sample18.cmd)]

V `bc.exe` içeren klasör, sistem s `PATH`içinde değilse, `%WINDOWS%\Microsoft.NET\Framework\version\`şöyle bir yola tam olarak başvurmanız gerekir:
...
> [!NOTE]
> Komut satırından `GetProductsWithPriceGreaterThan.vb` sınıf dosyası derlemek yerine, alternatif olarak [Visual Basic Express Edition](https://msdn.microsoft.com/vstudio/express/vb/) 'ı kullanabilir veya Visual Studio Standard Edition 'da ayrı bir sınıf kitaplığı projesi oluşturabilirsiniz. S Ren Jacob Lauritsen, `GetProductsWithPriceGreaterThan` saklı yordamı ve 3. adım ve 10. adımlarda oluşturulan iki yönetilen saklı yordam ve UDF için kod içeren bir Visual Basic Express Edition projesi sağladı. Ayrıca, ilgili veritabanı nesnelerini eklemek için gereken T-SQL komutlarını içeren s Ren Project.
...
Microsoft SQL Server 2005, yönetilen kod kullanılarak veritabanı nesnelerinin oluşturulmasına izin veren ortak dil çalışma zamanı (CLR) ile tümleştirme sağlar. Daha önce bu veritabanı nesneleri yalnızca T-SQL kullanılarak oluşturulabilir, ancak artık Visual Basic gibi .NET programlama dillerini kullanarak bu nesneleri oluşturmuyoruz. Bu öğreticide, yönetilen iki saklı yordam ve yönetilen Kullanıcı tanımlı bir Işlev oluşturduk.
