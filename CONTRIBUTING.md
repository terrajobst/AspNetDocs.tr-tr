...
* Açık bir [sorunu](https://github.com/aspnet/AspNetDocs/issues/new) açıklayan yeni bir tane oluşturmak veya mevcut bir makalede değiştirme gibi yapmak istediğiniz. Biz genellikle yeni bir konu öneri için bir ana hat isteyin. Kadar zaman yatırım önce Ekibi'nden gelen onay için bekleyin.
* Çatal [aspnet/AspNetDocs](https://github.com/aspnet/AspNetDocs/) depo ve yaptığınız değişiklikleri için bir dal oluşturun.
...
Makaleler yazılır [DocFx özellikli Markdown](https://dotnet.github.io/docfx/spec/docfx_flavored_markdown.html), bir alt kümesi olan [GitHub özellikli Markdown (GFM)](https://guides.github.com/features/mastering-markdown/). ASP.NET belgeleri yaygın olarak kullanılan kullanıcı Arabirimi özellikleri için DFM söz dizimi örnekleri için bkz. [meta veriler ve Markdown şablonu](https://github.com/dotnet/docs/blob/master/styleguide/template.md) .NET Docs deposuna bakın.

## <a name="folder-structure-conventions"></a>Klasör yapısı kuralları

Her bir Markdown dosyası için bir klasörü görüntüler ve örnek kod için bir klasör var. Makaleyi ise [signalr/overview/advanced/dependency-injection.md](https://github.com/aspnet/AspNetDocs/blob/master/aspnet/signalr/overview/advanced/dependency-injection.md), görüntüleri bulunan [signalr/genel bakış/Gelişmiş/bağımlılık-ekleme /\_statik](https://github.com/aspnet/AspNetDocs/tree/master/aspnet/signalr/overview/advanced/dependency-injection/_static) ve örnek uygulama projesi dosyalar, [signalr/genel bakış/Gelişmiş/bağımlılık-ekleme/samples](https://github.com/aspnet/AspNetDocs/tree/master/aspnet/signalr/overview/advanced/dependency-injection/samples). Görüntüdeki *signalr/overview/advanced/dependency-injection.md* dosya aşağıdaki Markdown tarafından işlenir:
...
* Komut kabuğu'na gidin *aspnet* içeren klasörü *docfx.json* dosyasını açıp aşağıdaki komutu çalıştırın:
...
Hedefimiz, geniş bir olası hedef kitlesi tarafından kolayca anlaşılır belgeleri yazmaktır. Bu amaçla, biz izlemenizi katkıda bulunanlar isteriz stili yazma yönergeleri kurdu. Daha fazla bilgi için [ses ve sesi yönergeleri](https://github.com/dotnet/docs/blob/master/styleguide/voice-tone.md) .NET deposundaki.
